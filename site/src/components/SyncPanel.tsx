...
import { Copy, Check, RefreshCw, AlertCircle, ChevronDown } from "lucide-react";
...
export function SyncPanel({ onSyncImport }: SyncPanelProps) {
  ...
  return (
    <div className="border-b border-border bg-card/20 backdrop-blur-sm">
      ...
      <button
        onClick={() => setOpen((v) => !v)}
        className="w-full flex items-center justify-between px-4 sm:px-6 py-2.5 text-[10px] text-muted-foreground/50 hover:text-muted-foreground tracking-widest uppercase transition-colors"
      >
        <span>クラウド同期 (Sync ID)</span>
        <ChevronDown
      ...
      {/* 展開コンテンツ */}
      {open && (
        <div className="px-4 sm:px-6 pb-4 max-w-3xl mx-auto space-y-4">
          {/* Sync ID 表示 */}
          <div>
