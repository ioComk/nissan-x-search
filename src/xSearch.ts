...
- 各ポストの要約テキスト全体をMarkdownリンク形式にする。必ず「[要約テキスト](https://x.com/...)」の形式のみで記述し、「要約テキスト。[要約テキスト](https://x.com/...)」のようにプレーンテキストとリンクを重複させてはいけない
- 日本語で出力する
- ユーザー名・ユーザーID（@xxx）は一切含めない
- いいね数・リポスト数・エンゲージメント数は一切含めない（❤️🔁などの絵文字も不要）
- ポストIDは含めない
- Markdown形式で見やすく整形する
- タイトル行（「日産自動車関連Xポスト要約レポート」など）は不要
- 冒頭に「## 概要」というセクションを設け、全体を2〜3文で説明し、ポジティブ/ネガティブの全体的な傾向にも触れる
